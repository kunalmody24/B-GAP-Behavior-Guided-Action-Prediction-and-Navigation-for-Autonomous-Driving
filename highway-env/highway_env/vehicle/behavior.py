...
class LinearVehicle(IDMVehicle):
    ...
    def acceleration_features(self, ego_vehicle, front_vehicle=None, rear_vehicle=None):
        ...
        if ego_vehicle:
            ...
            if front_vehicle:
                d = ego_vehicle.lane_distance_to(front_vehicle)
                dv = min(front_vehicle.velocity - ego_vehicle.velocity, -2.0)
                dp = min(d - d_safe, 0)
...
class AggressiveCar(ControlledVehicle):
    ...
    def act(self, action=None):
        ...
        action['acceleration'] = self.acceleration(ego_vehicle=self,
                                                   front_vehicle=front_vehicle,
                                                   rear_vehicle=rear_vehicle)
        if (self.target_velocity == 0 and self.lane_distance_to(front_vehicle) >= self.LENGTH):
